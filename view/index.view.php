...
$car = (array)$car; $first = array_shift($car);array_push($car, $first);
$motorcycle = (array)$motorcycle; $first = array_shift($motorcycle); array_push($motorcycle, $first);
$bicycle = (array)$bicycle; $first = array_shift($bicycle); array_push($bicycle, $first);
echo "<h2 style='text-align: center; color:#e8bfbf;'>TRANSPORTO PRIEMONĖS</h2>";
//var_dump($motorcycle);


echo '<br><table class="table table-hover table-bordered table-sm table-striped w-75" style="margin-left: auto; margin-right: auto; border: solid 2px; background-color: #EDEDED; font-size: 14px;"> 
...
<th>Kilmės šalis</th><th>Keleivių sk.</th><th>Specifinė sav.</th></tr>
</thead>
<tbody  class="table-group-divider text-center table-light"  style="border: solid 1px; font-size: small; vertical-align: middle">';
foreach ($car as $key=>$value){echo "<td>$value</td>";}
echo "</tr><tr>"; foreach ($motorcycle as $key=>$value){echo "<td>$value</td>";}
echo "</tr><tr>"; foreach ($bicycle as $key=>$value){echo "<td>$value</td>";}
echo '</tbody></table>';
?>
